...
def _annotate_OTGFT(
    ...
    **kw_args,
    ...
    fm_domain_adata = spg.calculate_frequcncy_domain(adata_copy,ratio_neighbors=1, spatial_info=adata_copy.obs_keys(),ratio_low_freq=ratio_low_adata,ratio_high_freq=ratio_high_adata)
    fm_domain_cell_type_read = spg.calculate_frequcncy_domain(cell_type_read,ratio_neighbors=1,spatial_info=adata_copy.obs_keys(),ratio_low_freq=ratio_low_adata,ratio_high_freq=ratio_high_adata)
    gene_df_adata = spg.detect_svg(adata_copy,
                            ...
                            filter_peaks=True,
                            S=kw_args['S_spagft'])
    svp_list_adata = gene_df_adata[gene_df_adata.cutoff_gft_score][gene_df_adata.qvalue < 0.05].index.tolist()
